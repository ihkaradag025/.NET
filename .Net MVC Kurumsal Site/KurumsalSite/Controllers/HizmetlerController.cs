        ...
        public ActionResult Create(Hizmet hizmet, HttpPostedFileBase ResimURL)
        {
            ...
            if (ModelState.IsValid)
            {
                if (ResimURL != null)
                {
                    ...
                    //} güncelleme işlemi olmadığı için kapattım
                    WebImage img = new WebImage(ResimURL.InputStream);
                    FileInfo imginfo = new FileInfo(ResimURL.FileName);

                    string logoname = imginfo.Name;  /*burada FullName kullanılırsa dosya yolu ile birlikte alır.burada otomatik isim vermesi için Guid.NewGuid().ToSting(); kullanılabilir*/
                    img.Resize(500, 500);
