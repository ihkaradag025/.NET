...
namespace DataAccessLayer
{
    public class DALOgrenci
    {
        ...
        public static List<EntityOgrenci> OgrenciListesi()
        {
            ...
            SqlDataReader dr = komut2.ExecuteReader();///sqldatareader sınıfından bir nesne ürettik bununla verileri okuyoruz
            while (dr.Read()) ///okumaya devam ettiği sürece
            {
                EntityOgrenci ent = new EntityOgrenci();//EntityOgrenci deki verilere ulaşmak için oluşturuldu
        ...
        public static bool OgrenciSil (int parametreSil)
        {
            ...
            komut3.Parameters.AddWithValue("@p1", parametreSil);
            return komut3.ExecuteNonQuery() > 0;
        }


        public static List<EntityOgrenci> OgrenciGuncelleListe(int parametreGuncelle)
        {
            List<EntityOgrenci> degerler = new List<EntityOgrenci>();
            SqlCommand komut4 = new SqlCommand("Select * From TBLOGRENCI where OGRID = @p1", Baglanti.bgl);
            komut4.Parameters.AddWithValue("@p1", parametreGuncelle);
            if (komut4.Connection.State != ConnectionState.Open)
            {
                komut4.Connection.Open();
            }
            SqlDataReader dr = komut4.ExecuteReader();///sqldatareader sınıfından bir nesne ürettik bununla verileri okuyoruz
            while (dr.Read()) ///okumaya devam ettiği sürece
            {
                EntityOgrenci ent = new EntityOgrenci();//EntityOgrenci deki verilere ulaşmak için oluşturuldu
